...
typedef enum _ACCESSORY_DEMO_COMMANDS
{
    ...
    COMMAND_UPDATE_LOG          = 0x04,
    COMMAND_REG_SETTING         = 0x05,
...
static void ADK_Task (void *pvParameters );
/****************************************************************************
  Function:
    int main(void)
...
static void vTask_test4 (void *pvParameters )
{
    ...
    for( ;; )
    {
        ...
        // vTaskDelay( 1000 );
        logNeedsUpdate = TRUE;
        if (gyro_id == Gyro_MPU6050_ID)
        {
            if (a) {
                temp = Gyro_MPU6050_GetData(ACCEL_XOUT_H);
                if (temp != acc_x) {
                    acc_x = temp;
                    //Lcd_1602_Display10BitData(acc_x,2,0);
                    if (temp > 0)
                        b = MOTOR_CCW;
                    else
                        b = MOTOR_CW;
                    StepMoter_28BYJ48_Direction(b);

                }
#if 0
                temp = Gyro_MPU6050_GetData(ACCEL_YOUT_H);
                if (temp != acc_y) {
                    acc_y = temp;
                    Lcd_1602_Display10BitData(acc_y,7,0);
                }
                temp = Gyro_MPU6050_GetData(ACCEL_ZOUT_H);
                if (temp != acc_z) {
                    acc_z = temp;
                    Lcd_1602_Display10BitData(acc_z,12,0);
                }
#endif
            } else {
                temp = Gyro_MPU6050_GetData(GYRO_XOUT_H);
                if (temp != gyro_x) {
                    gyro_x = temp;
                    //Lcd_1602_Display10BitData(gyro_x,2,1);
                }
#if 0
                temp = Gyro_MPU6050_GetData(GYRO_YOUT_H);
                if (temp != gyro_y) {
                    gyro_y = temp;
                    Lcd_1602_Display10BitData(gyro_y,7,1);
                }

                temp = Gyro_MPU6050_GetData(GYRO_ZOUT_H);
                if (temp != gyro_z) {
                    gyro_z = temp;
                    Lcd_1602_Display10BitData(gyro_z,12,1);
                }
#endif
            }
            a = 1-a;
...
static void ADK_Task (void *pvParameters )
{
    ...
    while(1)
    {
        ...
        // **************************************************************
        // **
        // ** Process the button / POT value update to Android tablet site, Regis
        // **
        // **************************************************************
        ...
        if((logNeedsUpdate == TRUE) && (writeInProgress == FALSE))
        {
            outgoing_packet.command = COMMAND_UPDATE_LOG;
            outgoing_packet.data = log_value;
